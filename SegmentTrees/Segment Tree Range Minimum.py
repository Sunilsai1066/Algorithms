class MinSegmentTree:
    ...
    def __init__(self, InArr):
        ...
        def BuildSegTree(Root, Start, End):
            ...
            self.SegTree[Root] = min(self.SegTree[(Root*2)+1], self.SegTree[(Root*2)+2])
            
        BuildSegTree(0, 0, len(InArr)-1)
        
    def RangeMinimum(self, Left, Right):
        def FindRangeMin(Root, Left, Right, Start, End):
            if(Start > Right or End < Left):
                return float('inf')
            elif(Start >= Left and End <= Right):
                return self.SegTree[Root]
            Mid = (Start + End)//2
            LVal = FindRangeMin((Root*2)+1, Left, Right, Start, Mid)
            RVal = FindRangeMin((Root*2)+2, Left, Right, Mid+1, End)
            return min(LVal, RVal)
            
        return FindRangeMin(0, Left, Right, 0, len(self.InArr)-1)
